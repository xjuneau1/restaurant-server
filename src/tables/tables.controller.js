...
async function updateTable(req, res, next) {
  ...
  const data = await service.updateTable(req.body.data, table_id);
  res.status(200).json({ data })
}
...
async function removeTable(req, res, next){
  const { table_id } = req.params
  await service.removeTable(table_id)
  res.sendStatus(204)
}
...
const VALID_PROPERTIES = ["table_id", "table_status", "created_at", "updated_at", "table_name", "capacity", "reservation_id"];
...
module.exports = {
  ...
  removeTable: [
    asyncErrorBoundary(isValidTableId),
    checkTableOccupied,
    asyncErrorBoundary(removeTable)
  ]
};
